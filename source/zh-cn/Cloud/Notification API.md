...
| data.events	| 必须 	|客户端接收消息类型，使用逗号隔开的字符串列表，目前支持类型为device.attr_fault,|

device.attr_alert,device.online,device.offline,device.status.raw,device.status.kv,datapoints.changed
